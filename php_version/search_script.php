...
<html>
    ...
    <body>
        ...
        <!-- Start of header for Nav. Header nav is there as a wrapper for the nav since
            ...
            $search_term = mysqli_real_escape_string($dbconnect, $search_term);
            
            //inner join LIKE means show what has those characters in their hero names
            ...
            INNER JOIN abilities ON abilities.Hero_ID = hero.Hero_ID AND hero.Hero_Name LIKE '%$search_term%')"); 
                ?>
                
                <header class="about-title-text-container">
                    <!-- display the hero name-->
                    <h1 class="header-text"><?php echo "You searched for '$search_term'."; ?></h1>
                </header>
